...
import { ArrowLeft, CheckCircle, XCircle, Printer, Package, FileText, Loader2, Trash2, Calendar as CalendarIcon, Edit2, Download } from 'lucide-react'
...
export default function WorkOrderDetailPage() {
  ...
  async function handleBluexExport() {
    try {
      setActionLoading(true)
      const workOrderId = Array.isArray(params.id) ? params.id[0] : params.id
      const response = await fetch(`/api/work-orders/${workOrderId}/export-bluex`)

      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || 'Error al exportar a Bluex')
      }
      ...
      a.href = url
      a.download = `Bluex_OT_${workOrder?.year}-${String(workOrder?.month).padStart(2, '0')}_${workOrderId.substring(0, 8)}.xlsx`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Error al exportar a Bluex')
    } finally {
      setActionLoading(false)
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {(workOrder.deliveryType === 'DOMICILIO' || workOrder.deliveryType === 'Delivery') && (
                <button
                  onClick={handleBluexExport}
                  disabled={actionLoading}
                  className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50"
                >
                  <Download className="w-4 h-4" />
                  {actionLoading ? 'Exportando...' : 'Exportar Bluex'}
                </button>
              )}
            </div>
          </div>

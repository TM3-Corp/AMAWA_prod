...
import { ArrowLeft, ChevronLeft, ChevronRight, CheckCircle, AlertCircle, Package, TrendingUp, Loader2, Plus, User } from 'lucide-react'
...
export default function MonthlyCalendarPage() {
  ...
  const [stockProjection, setStockProjection] = useState<StockProjection | null>(null)
  ...
  async function fetchStockProjection() {
    try {
      const response = await fetch('/api/inventory/stock-projection')
      if (response.ok) {
        const data = await response.json()
        if (data.success) {
          setStockProjection(data)
        }
      }
    } catch (err) {
      console.error('Error loading stock projection:', err)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {stockProjection?.summary.hasCriticalWarning && (
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-red-900">
                  ⚠️ Alerta Crítica de Inventario
                </h3>
                <p className="text-red-800 mt-1">
                  {stockProjection.summary.message}
                </p>
                <div className="mt-3 space-y-2">
                  {Object.values(stockProjection.filterCoverage)
                    .filter(f => f.isCritical)
                    .map(filter => (
                      <div key={filter.sku} className="bg-white rounded-lg p-3 border border-red-200">
                        <div className="flex items-center justify-between">
                          <div>
                            <span className="font-semibold text-gray-900">{filter.sku}</span>
                            <span className="text-gray-600 text-sm ml-2">{filter.name}</span>
                          </div>
                          <div className="text-right">
                            <div className="text-red-700 font-bold">
                              {filter.monthsUntilStockout} {filter.monthsUntilStockout === 1 ? 'mes' : 'meses'} de cobertura
                            </div>
                            <div className="text-sm text-gray-600">
                              Stock: {filter.currentStock} | Necesario: {filter.totalFutureConsumption}
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <Link
                  href="/inventory"
                  className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium"
                >
                    ...
                    <>
                        ...
                        <div className="space-y-2">
                          {deliveryData && deliveryData.deliveryCount > 0 && (
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-bold text-blue-600">{deliveryData.deliveryCount} ({percentages.delivery}%)</span>
                            </div>
                          )}
                          {presencialData && presencialData.presencialCount > 0 && (
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-700">👨‍🔧 Presencial</span>
                              <span className="text-sm font-bold text-green-600">{presencialData.presencialCount} ({percentages.presencial}%)</span>
                            </div>
                          )}
                        </div>
                      </div>

                      {/* Domicilio Work Order Status */}
                      {deliveryData && deliveryData.deliveryCount > 0 && (
                        <div className="border-t border-gray-200 pt-4 mb-4">
                          <p className="text-xs font-semibold text-gray-500 uppercase mb-2">📦 Domicilio - Orden de Trabajo</p>
                          {deliveryData.workOrder && deliveryData.workOrder.status === 'GENERATED' ? (
                            <Link
                            ...
                            >
                              ...
                              <span className="font-medium">OT Generada</span>
                            </Link>
                          ) : deliveryData.workOrder && deliveryData.workOrder.status === 'DRAFT' ? (
                            <Link
                              href={`/work-orders/${deliveryData.workOrder.id}`}
                              className="flex items-center gap-2 text-sm text-blue-700 bg-blue-50 rounded-lg p-3 hover:bg-blue-100 transition-colors"
                            >
                              <Package className="w-4 h-4" />
                              <span className="font-medium">OT en Borrador</span>
                            </Link>
                          ) : deliveryData.workOrder && deliveryData.workOrder.status === 'CANCELLED' ? (
                            <button
                              ...
                              className="w-full flex items-center justify-center gap-2 text-sm text-red-700 bg-red-50 rounded-lg p-3 hover:bg-red-100 transition-colors disabled:opacity-50"
                            >
                              {generatingWorkOrder === `${currentYear}-${monthNumber}-Delivery` ? (
                                ...
                                <>
                                  ...
                                  <span className="font-medium">OT Cancelada - Regenerar</span>
                                </>
                              )}
                            </button>
                          ) : (
                            ...
                            >
                              {generatingWorkOrder === `${currentYear}-${monthNumber}-Delivery` ? (
                                ...
                                <>
                                  ...
                                  <span className="font-medium">Generar OT</span>
                                </>
                              )}
                            </button>
                          )}
                      ...
                      {presencialData && presencialData.presencialCount > 0 && (
                        <div className="border-t border-gray-200 pt-4">
                          <p className="text-xs font-semibold text-gray-500 uppercase mb-2">👨‍🔧 Presencial - Asignación</p>
                          <Link
                            href={`/presencial?month=${monthNumber}&year=${currentYear}`}
                            className="w-full flex items-center justify-center gap-2 text-sm text-purple-700 bg-purple-50 rounded-lg p-3 hover:bg-purple-100 transition-colors"
                          >
                            <User className="w-4 h-4" />
                            <span className="font-medium">Asignar {presencialData.presencialCount} Mantención(es)</span>
                          </Link>
                          <p className="text-xs text-gray-500 mt-2 text-center">
                            Agrupadas por comuna
                          </p>
                        </div>
                      )}
                    </>
